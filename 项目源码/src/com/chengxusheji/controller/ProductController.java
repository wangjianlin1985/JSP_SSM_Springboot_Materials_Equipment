...
public class ProductController extends BaseController {
	...
	public void list(String productName,@ModelAttribute("supplierObj") Supplier supplierObj,@ModelAttribute("companyObj") Company companyObj,String luruTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (productName == null) productName = "";
		if (luruTime == null) luruTime = "";
		if(rows != 0)productService.setRows(rows);
		List<Product> productList = productService.queryProduct(productName, supplierObj, companyObj, luruTime, page);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(productName, supplierObj, companyObj, luruTime);
	    /*获取到总的页码数目*/
	...
	/*ajax方式按照查询条件分页查询物资信息*/
	@RequestMapping(value = { "/listAllWithFullInfo" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void listAllWithFullInfo(HttpServletResponse response) throws Exception {
		List<Product> productList = productService.queryAllProduct();
        response.setContentType("text/json;charset=UTF-8"); 
		PrintWriter out = response.getWriter();
		JSONArray jsonArray = new JSONArray();
		for(Product product:productList) {
			JSONObject jsonProduct = new JSONObject();
			jsonProduct.accumulate("productId", product.getProductId());
			jsonProduct.accumulate("productName", product.getProductName() + "-" + product.getSupplierObj().getSupplierName() + "-" + product.getCompanyObj().getCompanyName());
			jsonArray.put(jsonProduct);
		}
		out.println(jsonArray.toString());
		out.flush();
	...
	public String frontlist(String productName,@ModelAttribute("supplierObj") Supplier supplierObj,@ModelAttribute("companyObj") Company companyObj,String luruTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (productName == null) productName = "";
		if (luruTime == null) luruTime = "";
		List<Product> productList = productService.queryProduct(productName, supplierObj, companyObj, luruTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(productName, supplierObj, companyObj, luruTime);
	    /*获取到总的页码数目*/
	    int totalPage = productService.getTotalPage();
	...
	public void OutToExcel(String productName,@ModelAttribute("supplierObj") Supplier supplierObj,@ModelAttribute("companyObj") Company companyObj,String luruTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(productName == null) productName = "";
        if(luruTime == null) luruTime = "";
        List<Product> productList = productService.queryProduct(productName,supplierObj,companyObj,luruTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Product信息记录"; 
        String[] headers = { "物品id","物品名称","物品供应商","行政物需点","物品图片","物品价格","物品数量","录入时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<productList.size();i++) {
        	Product product = productList.get(i); 
        	dataset.add(new String[]{product.getProductId() + "",product.getProductName(),product.getSupplierObj().getSupplierName(),product.getCompanyObj().getCompanyName(),product.getProductPhoto(),product.getProductPrice() + "",product.getProductCount() + "",product.getLuruTime()});
        }
        /*
